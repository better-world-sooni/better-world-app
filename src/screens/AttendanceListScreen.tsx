...
const AttendanceListScreen = ({
  ...
  const {data: attendanceListRes, isLoading: attendanceListLoad} =
    useApiSelector(apis.attendance.collectionEventId.list);
  const {goBack} = useNavigation();
  const reloadGetWithToken = useReloadGETWithToken();
  const onRefresh = () => {
    if (attendanceListLoad) return;
    reloadGetWithToken(
  ...
  return (
    <Div flex={1} bgWhite>
      <Animated.FlatList
                ...
                <Row itemsCenter py5 h40 px8>
                  ...
                  <Col auto onPress={goBack}>
                    <Span bold fontSize={19}>
                      {attendanceCategory == AttendanceCategory.Yes
                        ? 'Yes'
                        : 'Maybe'}
                    </Span>
                  </Col>
        ...
        refreshControl={
          <RefreshControl
            refreshing={attendanceListLoad}
            onRefresh={onRefresh}
          />
        }
        data={attendanceListRes ? attendanceListRes.attendances : []}
