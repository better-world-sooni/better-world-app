...
const RepostListScreen = ({
  ...
  const {data: repostListRes, isLoading: repostListLoad} = useApiSelector(
    apis.post.postId.repost.list._,
  );
  const {goBack} = useNavigation();
  const reloadGetWithToken = useReloadGETWithToken();
  const onRefresh = () => {
    if (repostListLoad) return;
    reloadGetWithToken(apis.post.postId.repost.list._(postId, 1));
  };
  ...
  return (
    <Div flex={1} bgWhite>
      <Animated.FlatList
        ...
        refreshControl={
          <RefreshControl refreshing={repostListLoad} onRefresh={onRefresh} />
        }
...
function RepostOwner({nft, isFollowing}) {
