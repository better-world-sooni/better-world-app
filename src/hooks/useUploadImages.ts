...
export default function useUploadImages({
  ...
  const handleAddImages = async () => {
    try {
      const response = await MultipleImagePicker.openPicker({
        ...
        selectedColor: '#AA37FF',
      });
      const newFiles = await Promise.all(
        response.map(value => parseFileObject(value)),
      );
      setImages(newFiles);
      setError('');
  ...
  const parseFileObject = async file => {
    const base64 = file?.base64 ? file?.base64 : await fileBase64(file?.path);
    const fileObject = {
