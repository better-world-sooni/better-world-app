...
import { useDeletePromiseFnWithToken, usePatchPromiseFnWithToken } from "src/redux/asyncReducer";
...
export default function useUpdateDrawEvent({initialDrawEvent}){
    ...
    const updateDrawEventStatus = async (status) => {
        ...
        const {data} = await patchPromiseFn({url: apis.draw_event.drawEventId(drawEvent.id).url, body})
    ...
    const deleteDrawEvent = async () => {
        if(loading) return;
        setLoading(true)
        const {data} = await deletePromiseFn({url: apis.draw_event.drawEventId(drawEvent.id).url})
