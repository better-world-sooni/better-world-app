...
import { EventApplicationCategory, EventApplicationInputType } from "src/components/NewEventApplicationOptions";
...
export default function useUploadDrawEvent({initialHasApplication}){
    ...
    const { images, error, setError, handleAddImages, handleRemoveImage, uploadAllSelectedFiles } = useUploadImages({attachedRecord:"draw_event", fileLimit: 8})
    ...
    const uploadDrawEvent = async ({uploadSuccessCallback}) => {
		...
		const applicationOptions = applicationCategories.map((applicationCategory) => {
			if(applicationCategory.inputType !== EventApplicationInputType.SELECT) return {
	...
	const handleRemoveApplicationOption = (categoryIndex, optionIndex) => {
		const newApplicationCategories = [...applicationCategories]
		if(newApplicationCategories[categoryIndex]){
			newApplicationCategories[categoryIndex].options = newApplicationCategories[categoryIndex].options.slice(0, optionIndex).concat(newApplicationCategories[categoryIndex].options.slice(optionIndex+1))
			setApplicationCategories(newApplicationCategories)
		}
	}
