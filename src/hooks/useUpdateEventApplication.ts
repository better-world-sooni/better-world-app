...
export default function useUpdateEventApplication({initialEventApplication}){
    const [eventApplication, setEventApplication] = useState(initialEventApplication);
    ...
    const updateEventApplicationStatus = async (status) => {
        ...
        const {data} = await patchPromiseFn({url: apis.event_application.eventApplicationId(eventApplication.id).url, body})
