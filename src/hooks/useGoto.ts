...
export function useGotoNftCollectionProfile({nftCollection}) {
  ...
  const gotoProfile = () => {
    ...
    apiGETWithToken(
      apis.post.list.nftCollection(nftCollection.contract_address),
    );
    navigation.navigate(
...
export function useGotoCollectionSearch({contractAddress}) {
  ...
  const gotoCollectionEvent = () => {
    ...
    navigation.navigate(
      NAV_NAMES.CollectionSearch as never,
      {
        contractAddress,
      } as never,
    );
...
export function useGotoNewChatRoom() {
  const navigation = useNavigation();
  const apiGETWithToken = useApiGETWithToken();
  const gotoNewChatRoom = () => {
    apiGETWithToken(apis.chat.nftList());
    navigation.navigate(NAV_NAMES.NewChatRoom as never);
  };
  return gotoNewChatRoom;
}
