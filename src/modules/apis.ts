...
const BASE_URL = 'http://192.168.35.137:3000';
...
const apis = {
  ...
  feed: {
    ...
    draw_event: (filter?, page?) =>
      apiV1(`/feed/draw_event${urlParams({page, filter})}`),
  ...
  chat: {
    ...
    nftList: (keyword?, page?) =>
      apiV1(`/chat/nft_list${urlParams({page, keyword})}`),
  },
  collectionEvent: {
